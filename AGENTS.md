...
- Three local tiers: `pre-commit` (source policy only, about two seconds, run by the git pre-commit hook), `gate` (the broad check, run by the git pre-push hook), and `pre-release` (race, chaos, vulnerabilities, dependency freshness, API and Terraform; the release pipeline runs it and no hook fires it). `sparkwing pipeline hooks status` says whether the hooks fire in this checkout; a declared trigger is not an armed gate.
