...
- Three local tiers: `pre-commit` (source policy only, run by the git pre-commit hook; `DELIVERY.md` carries its measured cost), `gate` (the broad check, run by the git pre-push hook), and `pre-release` (race, chaos, vulnerabilities, dependency freshness, API and Terraform; the release pipeline and every hosted pull request run it, and no git hook fires it). `sparkwing pipeline hooks status` says whether the hooks fire in this checkout; a declared trigger is not an armed gate.
