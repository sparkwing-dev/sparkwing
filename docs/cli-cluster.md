...
per-principal guards bind a principal holding a metered token in its team.
max_concurrent_runners counts that team's claims, and max_runs_per_hour counts
that team's runs for the principal. The two max_global settings bind work across
teams. The three runner_scale settings raise max_concurrent_runners by one
