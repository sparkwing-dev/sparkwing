...
| Bytes the cache serves | 200 GiB a day per cache pod that verifies grants, unless the operator names another cap; with `--blob-store` the day's total survives a restart, without one it is per process | `cmd/sparkwing-cache/main.go`, `internal/cache/egress_day.go` |
