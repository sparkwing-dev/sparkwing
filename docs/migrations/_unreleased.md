...
- **After:** Schema 48 persists a hosted execution credential's run, root node,
  ...
  identity. Schema 49 records the one child output its parent may read. Older
  controllers refuse the store because they cannot enforce those bounds.
  Existing active metered claims are assigned to the principal that created
  their run during migration.
- **Migration:** Stop all controller replicas that share the store, upgrade them
  ...
  back after schema 48 or 49 is applied.
