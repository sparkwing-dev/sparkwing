...
| [v0.31.0](v0.31.0.md) | 2026-08-12 | `.Cache()` becomes `.Memoize()` on nodes and groups, with `CacheConfig`/`CacheOption`/`CacheConfig()` renamed to match, because the old name read like `actions/cache` while doing the opposite (skipping the node, not caching a directory). The `actions/cache` equivalent is the new `.CacheDir()`. Mechanical rename, no alias; every call site is a compile error until updated. |
