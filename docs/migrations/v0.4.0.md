...
The Workable struct convention -- drop the `Job` suffix
(`&BuildJob{}` → `&Build{}`) -- is a style preference, not a hard
break. The old name still compiles. The convention nudge is in the
godoc and embedded examples.
...
- Code that imported `secrets.Cipher` and passed it directly continues
  to work via Go's structural typing, **as long as the concrete type
  isn't referenced by name**. The Go field's type changed to the
  interface; concrete values still satisfy it.
- The `secrets.NewCipher` constructor isn't exported anymore. If you
  used it, you need either a custom Cipher implementation or to vendor
  the cipher code.
