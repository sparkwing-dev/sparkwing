...
- Each side ignores fields it does not know. A daemon sent a message type
  it does not serve answers `unsupported`, naming the type, and keeps the
  connection open; an unregistered route answers 404 with
  `{"error":"unsupported","route":"<method> <path>"}`. Neither is a
  dropped frame the caller has to time out on.
- A cut is a release decision. Removing or retyping a message field,
  removing a route, method, or response member, or raising the protocol
  floor needs a `(Breaking)` changelog entry and a migration section.

Two checks hold the rule up. `pkg/wingwire/testdata/shapes.json` records
...
`api/openapi.yaml`, and the protocol constants against the previous tag,
and refuses to cut a release whose changelog does not declare the cut.
