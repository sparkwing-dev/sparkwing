...
func TestValidateJobOverrides_UnknownNode(t *testing.T) {
	dir := t.TempDir()
	writeRunnersYAML(t, dir, `
runners:
  cloud-linux:
    type: kubernetes
    controller: shared
...
`)
	...
	sparkwing.Job(plan, "build", noopWork{})
	err := validateJobOverrides(Options{
		SparkwingDir:       filepath.Join(dir, ".sparkwing"),
		JobRunnerOverrides: map[string]string{"buld": "cloud-linux"},
	}, plan)
	if err == nil || !strings.Contains(err.Error(), "no plan node named \"buld\"") {
		t.Errorf("got %v", err)
	}
...
func TestValidateJobOverrides_UnknownRunner(t *testing.T) {
	dir := t.TempDir()
	writeRunnersYAML(t, dir, `runners: {}`)
	plan := sparkwing.NewPlan()
	sparkwing.Job(plan, "build", noopWork{})
	err := validateJobOverrides(Options{
		SparkwingDir:       filepath.Join(dir, ".sparkwing"),
		JobRunnerOverrides: map[string]string{"build": "ghost"},
	}, plan)
	if err == nil || !strings.Contains(err.Error(), "runner \"ghost\" is not declared") {
		t.Errorf("got %v", err)
	}
}

func TestValidateJobOverrides_RunnerMissesRequires(t *testing.T) {
	dir := t.TempDir()
	writeRunnersYAML(t, dir, `
runners:
  mac-mini:
...
`)
	plan := sparkwing.NewPlan()
	sparkwing.Job(plan, "build", noopWork{}).Requires("os=linux")
	err := validateJobOverrides(Options{
		SparkwingDir:       filepath.Join(dir, ".sparkwing"),
		JobRunnerOverrides: map[string]string{"build": "mac-mini"},
	}, plan)
...
func TestValidateJobOverrides_HappyPath(t *testing.T) {
	dir := t.TempDir()
	writeRunnersYAML(t, dir, `
runners:
  cloud-linux:
...
`)
	...
	err := validateJobOverrides(Options{
		SparkwingDir:       filepath.Join(dir, ".sparkwing"),
		JobRunnerOverrides: map[string]string{"build": "cloud-linux"},
	}, plan)
	if err != nil {
