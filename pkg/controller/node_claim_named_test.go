...
func TestClaimNodeByID_AwardsTheNamedNodeWithItsFence(t *testing.T) {
	...
	n, err := c.ClaimNodeByID(ctx, "run-1", "build", "k8s-job:sw-1", time.Minute, true)
	if err != nil {
...
func TestClaimNodeByID_RefusesANodeAnotherHolderHas(t *testing.T) {
	...
	if _, err := f.store.ClaimNamedNode(ctx, store.ClaimIdentity{},
		"run-1", "build", "agent:box-a", time.Minute, store.NamedClaimOptions{SizesToClass: true}); err != nil {
		t.Fatalf("seed the agent's claim: %v", err)
	}

	_, err := client.NewWithToken(f.url, nil, f.token).
		ClaimNodeByID(ctx, "run-1", "build", "k8s-job:sw-1", time.Minute, true)
...
func TestClaimNodeByID_ReportsANodeThatDoesNotExist(t *testing.T) {
	f := newNamedClaimFixture(t, store.TokenOptions{})
	_, err := client.NewWithToken(f.url, nil, f.token).
		ClaimNodeByID(context.Background(), "run-missing", "build", "k8s-job:sw-1", time.Minute, true)
	if err == nil {
...
func TestClaimNodeByID_NeedsTheClaimScope(t *testing.T) {
	...
	if _, err := client.NewWithToken(f.url, nil, reader).
		ClaimNodeByID(context.Background(), "run-1", "build", "k8s-job:sw-1", time.Minute, true); err == nil {
...
func TestClaimNodeByID_RefusesAnUnreadyNodeWithoutTheRunsDispatchClaim(t *testing.T) {
	...
	_, err := client.NewWithToken(f.url, nil, f.token).
		ClaimNodeByID(context.Background(), "run-1", "build", "rogue:pod", time.Minute, true)
	if !errors.Is(err, store.ErrLockHeld) {
...
func TestClaimNodeByID_AwardsAnUnreadyNodeToTheRunsDispatcher(t *testing.T) {
	...
	n, err := client.NewWithToken(f.url, nil, f.token).
		ClaimNodeByID(context.Background(), "run-1", "build", "k8s-job:sw-1", time.Minute, true)
	if err != nil {
...
func TestClaimNodeByID_RefusesALabelledNodeWithoutTheRunsDispatchClaim(t *testing.T) {
	...
	_, err := client.NewWithToken(f.url, nil, f.token).
		ClaimNodeByID(context.Background(), "run-1", "train", "unlabeled:pod", time.Minute, true)
	if !errors.Is(err, store.ErrLockHeld) {
...
func TestClaimNodeByID_AwardsALabelledNodeToTheRunsDispatcher(t *testing.T) {
	...
	n, err := client.NewWithToken(f.url, nil, f.token).
		ClaimNodeByID(context.Background(), "run-1", "train", "k8s-job:sw-1", time.Minute, true)
	if err != nil {
