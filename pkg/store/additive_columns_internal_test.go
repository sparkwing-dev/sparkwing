...
var additiveColumnSources = map[int][]map[string]string{
	...
	39: nil,
	// safety: v40 adds the creating-principal column with a default and
	// indexes, so an older binary keeps writing the migrated database.
	40: {runsPrincipalCols},
	...
	// safety: v47 adds the storage allowance and its charge watermark beside
	// the quota, and the team and bytes a storage charge billed, all
	// defaulted, so an older binary keeps writing the migrated database.
