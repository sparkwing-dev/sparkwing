...
func TestClaimNamedNodeAwardsAnUnqueuedNode(t *testing.T) {
	...
	n, err := s.ClaimNamedNode(ctx, store.ClaimIdentity{}, "run-named", "build", "k8s-job:sw-1", time.Minute)
	if err != nil {
...
func TestClaimNamedNodeRefusesANodeAnotherHolderHas(t *testing.T) {
	...
	_, err := s.ClaimNamedNode(ctx, store.ClaimIdentity{}, "run-taken", "build", "k8s-job:sw-1", time.Minute)
...
func TestClaimNamedNodeRefusesAFinishedNode(t *testing.T) {
	...
	_, err := s.ClaimNamedNode(ctx, store.ClaimIdentity{}, "run-done", "build", "k8s-job:sw-1", time.Minute)
	if !errors.Is(err, store.ErrLockHeld) {
...
func TestClaimNamedNodeReportsANodeThatDoesNotExist(t *testing.T) {
	...
	_, err := s.ClaimNamedNode(context.Background(), store.ClaimIdentity{},
		"run-missing", "build", "k8s-job:sw-1", time.Minute)
...
func TestClaimNamedNodeReservesCreditsForAMeteredToken(t *testing.T) {
	...
	if _, err := s.ClaimNamedNode(ctx, claimant, "run-billed", "build", "k8s-job:sw-1", time.Minute); err != nil {
		t.Fatalf("ClaimNamedNode: %v", err)
	}
...
func TestClaimNamedNodeIsRefusedWhenTheBalanceCannotCoverTheReservation(t *testing.T) {
	...
	_, err := s.ClaimNamedNode(ctx, claimant, "run-broke-named", "build", "k8s-job:sw-1", time.Minute)
