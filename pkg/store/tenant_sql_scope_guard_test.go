...
var unportedSQL = []string{
	...
	"(*Store).CountConcurrencyCache",
	"(*Store).CountDeadLocalConcurrency",
	...
	"(*Store).ForceReleaseSupersededHolders",
	"(*Store).GetActiveDebugPause",
	"(*Store).GetApproval",
	"(*Store).GetConcurrencyState",
	"(*Store).GetCronSchedule",
	"(*Store).GetGitHubWebhookBinding",
	"(*Store).GetLatestRun",
	"(*Store).GetNode",
	"(*Store).GetNodeDispatch",
	"(*Store).GetPipelineProfile",
	"(*Store).GetRun",
	...
	"(*Store).ListGitHubWebhookBindings",
	...
	"(*Store).ProfileSamples",
	"(*Store).PromoteNextWaiters",
	"(*Store).PruneEgressUsage",
	"(*Store).PruneRunsOlderThan",
	"(*Store).PurgeDeadLocalConcurrency",
	...
	"(*Store).ReleaseAndNotify",
	"(*Store).ReleaseClaimAtGeneration",
	"(*Store).ReleaseDebugPause",
	"(*Store).RepoForClaimedRun",
	"(*Store).ReposForClaimant",
	"(*Store).RequestCancel",
	...
	"(*Store).UpdatePlanSnapshot",
	"(*Store).UpsertProfilePin",
	...
	"(*Store).reapStaleConcurrencyHolders",
	"(*Store).reapStaleConcurrencyWaiters",
	"(*Store).reapStalePendingRuns",
	"(*Store).reapStaleRunningRuns",
	"(*Store).reapTimedOutApprovals",
	"(*Store).reconcileConcurrencyKeys",
	...
	"(*Store).sweepExpiredConcurrencyCache",
	"(*Store).sweepLRUConcurrencyCache",
	"(*Store).sweepOrphanedConcurrencyCache",
	"(*Store).sweepTable",
	...
	"txActiveHolders",
	"txCacheLookup",
	"txCheckConcurrencyInvariants",
	"txDeleteHolder",
	"txDeleteWaiter",
	"txDrainCoalesceFollowers",
	"txEarlierRunnableQueueWaiter",
	"txEntryCapacity",
	"txInsertHolder",
	"txLiveRunningRunIDs",
	"txLockEntry",
	"txNodeOutcome",
	"txPark",
	"txPromoteCoalesceWaiter",
	"txPromoteWaitersLocked",
	"txQueueWaiterArrivedAt",
	"txReapTerminalConcurrencyHolders",
	"txRefreshInheritedHolder",
	"txReleaseHolder",
	"txSupersede",
	"txSupersedeHolderAndInherited",
	"txTransferInheritedHolderCost",
...
func TestTenantSQLScope_MatcherAcceptsAndRefuses(t *testing.T) {
	cases := []struct {
		...
		{"insert with key", `INSERT INTO secrets (team, name, repo) VALUES (?,?,?)`, true},
		{"insert without key", `INSERT INTO secrets (name, repo) VALUES (?,?)`, false},
