...
SELECT `+runColumns+`
  FROM runs WHERE team = ? AND id = ?`, string(t.team), runID)
	...
	if err := t.s.loadAgentLossRetry(ctx, run); err != nil {
		return nil, err
	}
	return run, nil
}
...
func (t *Tenant) ListRuns(ctx context.Context, f RunFilter) ([]*Run, error) {
	return t.s.listRuns(ctx, t.team, f)
}
...
func (t *Tenant) CountRuns(ctx context.Context, f RunFilter) (int, error) {
	return t.s.countRuns(ctx, t.team, f)
}
...
func (t *Tenant) FinishRun(ctx context.Context, runID, status, errMsg string) error {
	...
	defer rollbackOrLog(tx)
	if err := t.s.assertRunMutationFenceTx(ctx, tx, runID); err != nil {
		return err
	}
...
// TouchRunHeartbeat stamps last_heartbeat_at=now on one of t's runs.
func (t *Tenant) TouchRunHeartbeat(ctx context.Context, runID string) error {
	...
	defer rollbackOrLog(tx)
	if err := t.s.assertRunHeartbeatFenceTx(ctx, tx, runID); err != nil {
		return err
	}
	if _, err := tx.ExecContext(ctx,
