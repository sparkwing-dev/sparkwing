...
func TestSchemaV56_RestatesTheRunnerScaleStepInTheNewCredit(t *testing.T) {
	cases := []struct {
		name string
		old  string
		want string
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			if _, err := seeded.DB().ExecContext(ctx,
				`DELETE FROM sparkwing_schema_version WHERE version >= 56`); err != nil {
				t.Fatalf("stamp v55: %v", err)
			}
			_ = seeded.Close()
			...
			// safety: the ladder records v56, so a second open must not scale the
...
func TestSchemaV56_LeavesAnUnsetStepUnset(t *testing.T) {
	target := storetest.New(t)
	seeded, err := target.TryOpen()
	if err != nil {
		t.Fatalf("Open#1: %v", err)
