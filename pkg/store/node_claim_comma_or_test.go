...
func TestNodeClaim_CommaORTermClaimableByEitherAlternative(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			n, err := s.ClaimNextReadyNode(ctx, "runner-principal", "pod-1", 30*time.Second, tc.runner)
			if tc.wantClaim {
				if err != nil {
...
func TestNodeClaim_BareLabelCommaOR(t *testing.T) {
	...
	n, err := s.ClaimNextReadyNode(ctx, "runner-principal", "pod-fpga", 30*time.Second, []string{"fpga"})
	if err != nil {
