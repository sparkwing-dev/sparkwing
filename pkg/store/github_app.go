...
const githubAppTriggerCols = `repository_id, pipeline, repository, installation_id, on_push, on_tags, on_pull_request,
...
func (t *Tenant) scanGitHubAppTriggers(rows *sql.Rows) ([]GitHubAppTrigger, error) {
	...
	for rows.Next() {
		...
		var push, tags, pr int
		var created int64
		if err := rows.Scan(&tr.RepositoryID, &tr.Pipeline, &tr.Repository, &tr.InstallationID, &push, &tags, &pr,
			&tr.CreatedBy, &created); err != nil {
			return nil, err
		}
		tr.Push, tr.Tags, tr.PullRequest = push != 0, tags != 0, pr != 0
...
func (t *Tenant) PutGitHubAppTrigger(ctx context.Context, tr GitHubAppTrigger, now time.Time) (GitHubAppTrigger, error) {
	...
	if !ok || tr.RepositoryID <= 0 || strings.TrimSpace(tr.Pipeline) == "" || (!tr.Push && !tr.Tags && !tr.PullRequest) {
	...
	_, err := t.s.exec(ctx, `
		...
		ON CONFLICT (team, repository_id, pipeline) DO UPDATE SET
		    ...
		    on_push = excluded.on_push, on_tags = excluded.on_tags, on_pull_request = excluded.on_pull_request,
		    created_by = excluded.created_by, created_at = excluded.created_at`,
		string(t.team), tr.RepositoryID, tr.Pipeline, tr.Repository, tr.InstallationID, flag(tr.Push), flag(tr.Tags),
