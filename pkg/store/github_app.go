...
const githubAppTriggerCols = `repository_id, pipeline, repository, installation_id, on_push, on_pull_request,
       created_by, created_at, on_pr_closed, on_pr_labeled, pr_labels, on_pr_ready_for_review,
...
func (t *Tenant) scanGitHubAppTriggers(rows *sql.Rows) ([]GitHubAppTrigger, error) {
	...
	for rows.Next() {
		...
		if err := rows.Scan(&tr.RepositoryID, &tr.Pipeline, &tr.Repository, &tr.InstallationID, &push, &pr,
			&tr.CreatedBy, &created, &closed, &labeled, &labels, &ready, &published, &prereleased, &create, &deleted); err != nil {
			return nil, err
		}
...
func (t *Tenant) PutGitHubAppTrigger(ctx context.Context, tr GitHubAppTrigger, now time.Time) (GitHubAppTrigger, error) {
	...
	if !ok || tr.RepositoryID <= 0 || strings.TrimSpace(tr.Pipeline) == "" ||
		(!tr.Push && !tr.PullRequest && !tr.PullRequestClosed && !tr.PullRequestLabeled && !tr.PullRequestReadyForReview && !tr.ReleasePublished && !tr.ReleasePrereleased && !tr.BranchCreate && !tr.BranchDelete) {
	...
	_, err = t.s.exec(ctx, `
		...
		flag(tr.PullRequest), tr.CreatedBy, tr.CreatedAt.Unix(), flag(tr.PullRequestClosed), flag(tr.PullRequestLabeled),
		string(labels), flag(tr.PullRequestReadyForReview), flag(tr.ReleasePublished), flag(tr.ReleasePrereleased), flag(tr.BranchCreate), flag(tr.BranchDelete))
	if err != nil {
		return GitHubAppTrigger{}, err
	}
	return tr, nil
