...
func TestCoordinatorLocationIsReservedForTheDirectExecutor(t *testing.T) {
	...
	if err := s.EnrollExecutor(ctx, "swr_agent", store.Executor{
		...
		t.Fatal("an enrolled helper claimed the reserved coordinator location")
	}
	if err := s.EnrollExecutor(ctx, "swr_direct", store.Executor{
		Name: "laptop", Kind: "direct", Location: "coordinator", MaxConcurrent: 1,
	}); err != nil {
		t.Fatalf("enroll the foreground direct executor: %v", err)
	}
}
