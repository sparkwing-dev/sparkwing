...
func downgradeTokenPrefixIndex(t *testing.T, target *storetest.Target, seed ...[2]string) {
	t.Helper()
	...
	for _, row := range seed {
		if err := insertTokenRow(t, st, row[0], "swu_dupdupdu", row[1]); err != nil {
...
func insertTokenRow(t *testing.T, s *store.Store, hash, prefix, principal string) error {
	t.Helper()
	_, err := s.DB().Exec(
		storetest.Rebind(s, `INSERT INTO tokens (hash, prefix, principal, kind, scopes, created_at)
		 VALUES (?, ?, ?, ?, ?, ?)`),
...
func TestSchemaV26_MakesTheTokenPrefixIndexUniqueOnAnOlderDatabase(t *testing.T) {
	...
	downgradeTokenPrefixIndex(t, target)
	...
	indexSQL, err := indexDefinition(t, up, store.TokenPrefixIndexName)
	if err != nil {
	...
	if err := insertTokenRow(t, up, "argon2id$00$00", "swu_dupdupdu", "alice"); err != nil {
		t.Fatalf("seed row: %v", err)
	}
	if err := insertTokenRow(t, up, "argon2id$11$11", "swu_dupdupdu", "mallory"); err == nil {
