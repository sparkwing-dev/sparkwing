...
| `--sw-workers=N` | `runtime.NumCPU()` | Caps the local dispatcher, with or without `--sw-mode`. GHA hosted runners are 2-CPU, so `--sw-workers=4` over-subscribes a small VM -- pick deliberately. |
