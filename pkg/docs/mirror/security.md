...
Schema 30 supplies this process boundary as an internal release dependency. It
must ship with schema 31, which adds the current-attempt fence and durable
grants required by `Memoize`, `Concurrency`, `ToolSlot`, `RunAndAwait`,
cross-pipeline references, and dynamic `SpawnNode`. Schema 30 is not a complete
assisted-node compatibility boundary on its own.
