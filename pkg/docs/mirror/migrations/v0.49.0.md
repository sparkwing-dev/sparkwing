...
Five surfaces are cut in this release, each a hard cut with no shim: the retired
spelling fails naming its replacement. The sections below are independent, so
apply the ones you use in any order.
...
performs no service action. `serve` takes `start`, `stop`, `restart`, `logs`
and `status`; there is no `serve kill`.

`serve start` leaves a running service alone and reports it, where `dashboard
start` replaced whatever it found. A script that restarted the dashboard by
calling `start` again calls `serve restart`. Lifecycle receipts name the owner,
the effective endpoints, readiness, and the artifact the service runs, and
`serve logs` reads a bounded window of the service log.

The dashboard UI, HTTP routes, JSON service identity `dashboard`, and
