...
5. **Update CLI usage.** [cli-flag-renames](#cli-flag-renames),
   [cli-retired-flags](#cli-retired-flags),
   [cli-output-aliases](#cli-output-aliases), and
   [no-cache-env-rename](#no-cache-env-rename) cover scripts, CI, and
   shell aliases that pinned old flag names, the `--json` / `--pretty`
   output shortcuts, or the `SPARKWING_NO_CACHE` env var.
...
- `SPARKWING_NO_CACHE=1` set in an old shell config now silently
  bypasses the per-node result cache instead of the bincache.
  Behaviorally surprising if the operator expected the old meaning.
  Audit your environment configs.
- Compose scripts, CI configs, k8s ConfigMaps all need updating if
  they set `SPARKWING_NO_CACHE` expecting the old semantics.
