...
- **After:** Schema 48 persists a hosted execution credential's run, root node,
  ...
  identity. Older controllers refuse the store because they cannot enforce that
  binding. Existing active metered claims are assigned to the principal that
  created their run during migration.
- **Migration:** Stop all controller replicas that share the store, upgrade them
  ...
  client may continue sending an empty body. Do not roll a replica back after
  schema 48 is applied.
