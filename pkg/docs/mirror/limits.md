...
allowance is split into three fixed shares, one per store, and each store
holds the team to its share in the process that writes the bytes, against a
count that process keeps:

| Store | Share | Counted by | Checked |
|---|---|---|---|
| Cache: compiled binaries, dependency archives, artifacts | 3/4, 768 MiB | the cache's per-team count of its bucket | before the upload's body is read |
| Logs: live runs on the volume and the archive | 3/16, 192 MiB | the logs service's archive count plus the unarchived bytes of each team's live runs | after the node and run caps cut the append, before it is written |
| Run events | 1/16, 64 MiB | the controller's per-team event bytes | in the transaction that appends the event |

One store never reads another's count, so a team whose cache is full can
...
object remains. A write that fails for any reason gives its reservation back.

Two writes of one team in flight at once see each other's reservations. A
store counts a byte from the moment it is written until the moment it is
deleted: archiving a run moves its bytes from the volume's count to the
archive's, and only deleting them gives the room back.
...
the three shares and its event bytes under `team`.

Each store also exports `sparkwing_free_storage_used_bytes{store=...}`: what
teams last known to be free hold there, writes in flight included.
...
To oversubscribe, raise `--free-team-slots` and alert on
`sparkwing_free_storage_used_bytes`. A sign-up gate reads the tier as closed
once every slot is taken.

A multi-team controller refuses to start without `--bucket-store`, because the
counters that hold free teams to their shares live over the object store: the
...
| Team binaries, dependency archives and artifacts | written more than 30 days ago, deleted in the daily listing that reconciles the cache's count | cache | `--usage-reconcile`; the operator's own team keeps its objects |
...
| Logs, live and archived | the log share, 192 MiB by default | `pkg/logs/log_quota.go`, `pkg/logs/server.go` |
| Cache binaries, dependency archives and artifacts | the cache share, 768 MiB by default, refused before the body is read | `internal/cache/blobquota.go`, `internal/cache/blobstore.go` |
...
| Bytes the cache serves one team | 5 GiB a UTC day through the team's grants (`--egress-team-daily-free-bytes`), 50 GiB for a funded team (`--egress-team-daily-funded-bytes`); past it `429` until midnight UTC. With `--blob-store` the team's day survives a restart | `internal/cache/egress.go`, `internal/cache/egress_day.go` |
| Bytes the cache serves | 200 GiB a day per cache pod that verifies grants, unless the operator names another cap; with `--blob-store` the day's and the month's totals survive a restart, without one they are per process | `cmd/sparkwing-cache/main.go`, `internal/cache/egress_day.go` |
