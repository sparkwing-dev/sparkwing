...
Choosing an installation binds it to the active team. The controller checks the signed, unexpired state and PKCE verifier against the account and team, confirms the linked GitHub identity, reads the chosen installation with the App credential, repeats the admin check, and consumes the state in the store before binding. A state can bind only once, including across controller replicas. The authorization proof carried from the list to the selection is encrypted by the controller and expires with the state.
