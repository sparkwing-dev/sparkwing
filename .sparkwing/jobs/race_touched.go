...
func raceGoCommand(cpuCount int, pkgs []string) string {
	return boundedGoCommand(cpuCount, "test", "-tags e2e -race -count=1 -timeout 30m "+strings.Join(pkgs, " "))
}
...
func runRaceTouched(ctx context.Context) error {
	...
	return withGoTestScratch(func(testRoot string) error {
		...
		for _, module := range mapKeys(targets) {
			...
			cmd := raceGoCommand(runtime.NumCPU(), pkgs)
