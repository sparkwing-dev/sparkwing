...
func runJobsGrep(ctx context.Context, paths orchestrator.Paths, args []string) error {
	...
	resolvedFmt, err := resolveTTYAwareOutput(*outFmt, cmdJobsGrep.Path)
	if err != nil {
		return err
	}
	...
	return orchestrator.RunGrepRemote(ctx, prof.ControllerURL(), profileLogsURL(prof), prof.ControllerToken(), opts, os.Stdout)
}
