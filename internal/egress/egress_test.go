...
func TestFlushDayHandsOverTodaysTotalAndRestoreDayResumesIt(t *testing.T) {
	...
	if len(got) != 1 || got[0].Day != "2026-09-13" || got[0].Bytes != 1000 || got[0].Month != "2026-09" || got[0].MonthBytes != 1000 {
...
// A principal's daily cap refuses only that principal, answers with the wait
// until the UTC day rolls, and a write already under way stops at it.
func TestDailyCapOfOnePrincipalRefusesOnlyItUntilTheDayRolls(t *testing.T) {
	m, clock := meterAt(t, egress.Config{}, "2026-09-13T18:00:00Z")
	daily := egress.DailyCap{Bytes: 100, Remedy: "Add credits to raise it."}
	rec := httptest.NewRecorder()
	out := m.ServeDaily(rec, httptest.NewRequest(http.MethodGet, "/a", nil), "team:acme", egress.ClassArtifact, daily)
	n, err := out.Write([]byte(strings.Repeat("x", 160)))
	var budget *egress.BudgetError
	if !errors.As(err, &budget) || !budget.PrincipalDaily || n != 100 {
		t.Fatalf("a write past the principal's daily cap = %d, %v; want 100 sent and its daily *BudgetError", n, err)
	}
	err = m.CheckDaily("team:acme", daily)
	if !errors.As(err, &budget) || !errors.Is(err, egress.ErrBudgetExceeded) {
		t.Fatalf("CheckDaily at the cap = %v, want a *BudgetError", err)
	}
	if budget.RetryAfter != 6*time.Hour || budget.UsedBytes != 100 || budget.LimitBytes != 100 {
		t.Fatalf("refusal = %+v, want 100 of 100 and the 6h until midnight UTC", budget)
	}
	for _, want := range []string{"team:acme", "daily", "Add credits to raise it."} {
		if !strings.Contains(budget.Error(), want) {
			t.Errorf("refusal %q does not name %q", budget.Error(), want)
		}
	}
	if err := m.CheckDaily("team:other", daily); err != nil {
		t.Fatalf("another principal under the same cap = %v, want nil", err)
	}
	if err := m.Check("team:acme"); err != nil {
		t.Fatalf("Check without the daily cap = %v, want nil: the cap is the caller's to apply", err)
	}
	*clock = at(t, "2026-09-14T00:00:00Z")
	if err := m.CheckDaily("team:acme", daily); err != nil {
		t.Fatalf("CheckDaily after the day rolled = %v, want nil", err)
	}
}

// A restart resumes each principal's day, so a restart does not reopen a
// principal's daily cap; another day's totals are ignored.
func TestFlushDayCarriesEachPrincipalsDayAcrossARestart(t *testing.T) {
	m, _ := meterAt(t, egress.Config{}, "2026-09-13T10:00:00Z")
	m.Record("team:acme", egress.ClassArtifact, 90)
	m.Record("team:beta", egress.ClassGit, 5)
	var saved egress.DayUsage
	if err := m.FlushDay(func(u egress.DayUsage) error { saved = u; return nil }); err != nil {
		t.Fatal(err)
	}
	if saved.Principals["team:acme"] != 90 || saved.Principals["team:beta"] != 5 {
		t.Fatalf("saved principals = %v, want acme 90 and beta 5", saved.Principals)
	}
	daily := egress.DailyCap{Bytes: 90}

	restarted, _ := meterAt(t, egress.Config{}, "2026-09-13T11:00:00Z")
	if err := restarted.CheckDaily("team:acme", daily); err != nil {
		t.Fatalf("a fresh meter refused acme before any restore: %v", err)
	}
	restarted.RestoreDay(saved)
	if err := restarted.CheckDaily("team:acme", daily); !errors.Is(err, egress.ErrBudgetExceeded) {
		t.Fatalf("CheckDaily after restoring acme's spent day = %v, want the cap's refusal", err)
	}
	if err := restarted.CheckDaily("team:beta", daily); err != nil {
		t.Fatalf("beta under the cap after the restore = %v, want nil", err)
	}

	tomorrow, _ := meterAt(t, egress.Config{}, "2026-09-14T01:00:00Z")
	tomorrow.RestoreDay(saved)
	if err := tomorrow.CheckDaily("team:acme", daily); err != nil {
		t.Fatalf("yesterday's total closed today's cap: %v", err)
	}
}
