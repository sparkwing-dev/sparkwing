...
func TestWebhookGitHub_PushEnqueuesTrigger(t *testing.T) {
	...
	resp := postWebhook(t, ts.URL+"/webhooks/github/my-app-build", "push", body, signWebhook(testWebhookSecret, body))
	...
	if tr.Pipeline != "my-app-build" {
		t.Errorf("pipeline=%q want my-app-build", tr.Pipeline)
	}
	...
	if tr.TriggerEnv["GITHUB_REPOSITORY"] != "your-org/my-app" {
