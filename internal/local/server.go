// Package local is the laptop-mode control plane: an in-process HTTP
// service fronting the run/node/event/cache state store. localws
// embeds it on the same port as the dashboard + logs server.
//
// The handler set stays 1:1 with the cluster-mode controller so the
// dashboard frontend + CLI binary speak the same wire protocol against
// either side.
package local
...
type Server struct {
	...
	// artifactStore is populated by the harness before Handler() is
	// called. Zero value is valid -- the artifact route returns 404
...
func (s *Server) Handler() http.Handler {
	...
	// Artifact reads. Open to runs.read scope.
...
// ServeWith runs a pre-built Server (already configured with
// WithDispatcher) at addr. Split from Serve so the dispatcher can
// close over the Server itself before binding the listener.
