...
func Run(ctx context.Context, backends Backends, opts Options) (*Result, error) {
	...
	if !ok {
		...
		return nil, fmt.Errorf("pipeline %s is not defined in %s@%s; defined: %s",
			opts.Pipeline, repo, sha, defined)
	}
...
func (s *dispatchState) pipelineAwaiter() sparkwing.PipelineAwaiter {
	return sparkwing.PipelineAwaiterFunc(func(ctx context.Context, req sparkwing.AwaitRequest) (*sparkwing.ResolvedPipelineRef, error) {
		...
		localAdmission, _, _ := localAdmissionFromContext(ctx)
		config := childAwaitConfig{
			state:               s.backends.State,
			concurrency:         s.backends.Concurrency,
			parentRunID:         s.runID,
			retryOf:             s.retryOf,
			triggerEnv:          leaseTriggerEnv,
			masker:              s.masker,
