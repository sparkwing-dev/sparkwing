...
func Run(ctx context.Context, backends Backends, opts Options) (result *Result, returnedErr error) {
	...
	if !skipDispatch {
		if st := canonicalLocalStore(backends.State); st != nil {
			...
			recordRunProfile(finishCtx, st, opts.Pipeline, runID, planPin(plan), capacityFingerprint(plan), charge, contended, execStart, time.Now(), executionShapeHashes(plan.Nodes()))
		}
	}
