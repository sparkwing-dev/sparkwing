...
func TestLoopbackScopesProfileWritesToItsRunsPipeline(t *testing.T) {
	st := coordinationStore(t)
	ctx := context.Background()
	if err := st.CreateRun(ctx, store.Run{
		ID: "r1", Pipeline: "demo", Status: "running", StartedAt: time.Now(),
		DeclaredRepo: "acme/web", RepoURL: "https://github.com/acme/web.git",
	}); err != nil {
		t.Fatalf("create run: %v", err)
	}
	c := newCoordinationLoopback(t, st)
	...
	if err := c.RecordProfileObservation(ctx, own, "", measurement); err != nil {
		t.Fatalf("observation on this run's own pipeline: %v", err)
	}
	if err := c.SetPipelinePin(ctx, own, "", 4, 1<<30); err != nil {
		t.Fatalf("pin on this run's own pipeline: %v", err)
	}
	if err := c.RecordProfileObservation(ctx, other, "", measurement); err == nil {
		t.Error("wrote another pipeline's profile through a loopback bound to this run")
	}
	if err := c.SetPipelinePin(ctx, other, "", 4, 1<<30); err == nil {
		t.Error("pinned another pipeline through a loopback bound to this run")
	}
	if prof, err := st.GetPipelineProfile(ctx, other, ""); err != nil || prof != nil {
		t.Errorf("other-pipeline profile = %v (err %v), want none", prof, err)
	}
