...
func TestProxyAllowList_SessionScopesGateProxiedRoutes(t *testing.T) {
	...
	for _, test := range []struct {
		...
		{"writer pushes crons", []string{controller.ScopeRunsWrite}, http.MethodPut, "/api/v1/crons/repos", http.StatusNoContent},
		{"writer runs a cron", []string{controller.ScopeRunsWrite}, http.MethodPost, "/api/v1/crons/crn_1/run", http.StatusNoContent},
		{"writer clears an override", []string{controller.ScopeRunsWrite}, http.MethodDelete, "/api/v1/crons/crn_1/override", http.StatusNoContent},
