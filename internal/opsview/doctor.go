...
type DoctorReport struct {
	...
	UnreachablePeers []DoctorUnreachablePeer `json:"unreachable_peers,omitempty"`
...
// DoctorUnreachablePeer is a daemon socket belonging to another sparkwing
// home that took the connection and then failed the handshake. The sweep
// records it so a machine holding wedged peer daemons does not read as one
// holding none.
type DoctorUnreachablePeer struct {
	Socket string `json:"socket"`
	...
	for _, sock := range socks {
		...
		if err != nil {
			if fault := confirmedPeerFault(ctx, sock, err); fault != nil {
				report.UnreachablePeers = append(report.UnreachablePeers,
					DoctorUnreachablePeer{Socket: sock, Error: fault.Error()})
			}
			continue
		}
...
func confirmedPeerFault(ctx context.Context, sock string, err error) error {
	...
	if _, second := wingdclient.Probe(ctx, sock); !acceptedThenFailed(ctx, second) {
		return nil
	}
	return err
}
...
func renderDoctorPlain(w io.Writer, r DoctorReport) error {
	...
	fmt.Fprintf(w, "unreachable_peers\t%d\n", len(r.UnreachablePeers))
	competing := 0
...
func renderUnreachablePeers(w io.Writer, r DoctorReport) {
