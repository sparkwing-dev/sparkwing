...
func TestApplyHostCeiling(t *testing.T) {
	cases := []struct {
		...
		{
			name:       "measured memory over the ceiling clamps silently",
		...
		{
			name:       "memory pin over the ceiling stays hard",
			res:        Resolution{MemoryBytes: 32 << 30, Source: store.CostSourcePin},
			machine:    10,
			grantCores: 8,
			grantMem:   16 << 30,
			wantMem:    32 << 30,
		...
		{
			name:        "capped floor memory names the poisoned profile and reset",
			res:         Resolution{Cores: 1, MemoryBytes: 32 << 30, Source: store.CostSourceFloor},
			machine:     8,
			grantCores:  7.5,
			grantMem:    16 << 30,
			wantCores:   1,
			wantMem:     16 << 30,
			wantWarning: "measuring charge 32GB exceeds this machine's grantable 16GB, so runs are admitted alone; if contention poisoned the profile, reset it: sparkwing runs stats --reset --pipeline repo/ci",
		},
		{
			name:       "unknown memory ceiling leaves the charge",
			res:        Resolution{MemoryBytes: 32 << 30, Source: store.CostSourceMeasured},
			grantCores: 8,
			wantMem:    32 << 30,
		},
		{
