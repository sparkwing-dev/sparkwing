...
//
// Lending the run a stable alias -- a symlink every worktree lints
// through -- does not rescue it either. git resolves the alias and
// reports the real worktree as the repository root, so findings the
// linter recorded under the alias sit outside the diff and a baseline
// such as golangci-lint's new-from-merge-base drops every one: a tree
// with eight findings lints clean. A per-worktree cache is the whole
// answer. A fixed-workdir runner already has a stable path, and
// [RestoreLintCache] seeds this directory for it.
//
