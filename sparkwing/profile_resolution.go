...
// args-resolver needs at registration-invoke time: the active
// profile name (so [Profile]("prod") predicates fire) and whether
// that profile is local-only (so [Local] / [Remote] gates resolve
// correctly).
//
// Installed on ctx by the orchestrator via
// internal/sparkwingruntime.WithProfileResolution and read back by
// the sparkwing package's invoke() before it calls Schema.Resolve.
// Pipeline authors don't construct or read this type directly.
//
// Per-arg defaulting lives in pipeline YAML (the redesign moved it
// off profiles, since defaults are deployment-binding policy rather
// than dispatch-destination policy).
